...
using System.Text;
using CreateMeasurements;

//Number of entries
const double size = 1e9;

List<WeatherStation> stations = [new WeatherStation("Abha", 18.0),
...
await using (StreamWriter outputFile = new(Path.Combine("measurements.txt"), true, Encoding.UTF8, 1048576))
{
    for (var i = 0; i < size; i++)
    {
        var station = stations[rand.Next(stations.Count)];
        await outputFile.WriteAsync(station.Id);
